Foodkart:
Description:
Flipkart is starting a new online food ordering service. In this Service, users can order food from a restaurant which is serviceable 
in their area and the restaurant will deliver it.
Features:
...
- Users should be able to login, and all the operations will happen in the context of that user. If another user logged in, the previous user will 
...
register_user(name, gender, phoneNumber(unique) and pincode)
login_user(user_id)
Register_restaurant(resturant_name, list of serviceable pin-codes, food item  name, food item price, initial quantity)
update_quantity(restaurant name, quantity to Add)
rate_restaurant(restaurant name, rating, comment)
